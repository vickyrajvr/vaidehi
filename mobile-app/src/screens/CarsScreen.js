...
export default function CarsScreen(props) {
    ...
    React.useEffect(() => {
        props.navigation.setOptions({
            headerRight: () => {
                return (
                    <TouchableOpacity onPress={() => props.navigation.navigate('CarEdit')} style={{ marginEnd: 8, width: 80, alignItems: 'flex-end', padding: 5 }}><Text style={[styles.headerTitleStyle, { color: colors.WHITE }]}>{t('add')}</Text></TouchableOpacity>
                )
            }
        });
    }, [props.navigation]);
    ...
    return (
        <ScrollView styles={styles.container} showsVerticalScrollIndicator={false}>
                        ...
                        <TouchableWithoutFeedback key={"index" + i} onPress={() => onPress(c)}>
                            <View style={styles.card}>
                                ...
                                <View style={{ width: '100%', flexDirection: isRTL ? 'row-reverse' : 'row',height:180 }}>
                                    ...
                                    <View style={{ flexDirection: 'column', width: '40%', justifyContent: 'center', alignItems: 'center' }}>
                                        <View style={styles.vew2}>
                                            ...
                                            <Text style={[styles.text3,{fontWeight:'bold'}]}>{c.approved ? t('yes') : t('no')}</Text>
                                        </View>
                                        <View style={styles.vew2}>
                                            <Text style={styles.text3}>{t('active_status')}</Text>
                                            <Text style={[styles.text3,{fontWeight:'bold'}]}>{c.active ? t('yes') : t('no')}</Text>
                                        </View>
                                    </View>
                                </View>
                                ...
                                <View style={[styles.vew, { flexDirection: isRTL ? 'row-reverse' : 'row' }]}>
                                    ...
                                    <View>
                                        <Text style={styles.text1}>{t('vehicle_reg_no')}</Text>
                                        <Text style={[styles.text1,{fontWeight:'bold', textAlign: isRTL ? 'right' : 'left'}]}>{c.vehicleNumber}</Text>
                                    </View>
                                ...
                                <View style={[styles.vew, { flexDirection: isRTL ? 'row-reverse' : 'row',marginBottom:10}]}>
                                    ...
                                    <View>
                                        <Text style={styles.text1}>{t('other_info')}</Text>
                                        <Text style={[styles.text1,{fontWeight:'bold', textAlign: isRTL ? 'right' : 'left'}]}>{c.other_info}</Text>
                                    </View>
            ...
            <View style={{flex: 1, justifyContent:'center', alignItems:'center', height: height/2, padding: 10}}>
                <Text style={{fontSize: 23, fontWeight:'bold'}}>{t('car_add')}</Text>
            </View>
            }
        </ScrollView>
    )
